            ...
            for entry in module_data:
                ...
                if xml_var in ['LOCATION']:
                    ...
                    if entry['nested_query']:
                        query = entry['nested_query'] + f" WHERE {dbase_table}.sen_name = '{sen_name}' AND (xml_upload_success IS NULL OR xml_upload_success = FALSE);"
                        
                    else:
                        # Modify the query to get the latest entry
                        query = f"SELECT {dbase_col} FROM {dbase_table} WHERE sen_name = '{sen_name}' AND (xml_upload_success IS NULL OR xml_upload_success = FALSE);"
