...
async def process_module(conn, yaml_file, xml_file_path, output_dir, date_start, date_end, lxplus_username, partsnamelist=None, skip_uploaded=True):
    ...
    if partsnamelist:
        query = f"""SELECT REPLACE(bp_name,'-','') AS bp_name FROM bp_inspect WHERE bp_name = ANY($1) {skip_filter}"""
            ...
            for entry in wb_data:
                ...
                if xml_var in ['LOCATION']:
                    ...
                    if entry['nested_query']:
                        query = entry['nested_query'] + f" WHERE REPLACE({dbase_table}.bp_name,'-','') = '{bp_name}' ;"
