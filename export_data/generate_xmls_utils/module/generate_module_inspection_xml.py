            ...
            for entry in module_data:
                ...
                if xml_var in ['LOCATION']:
                    ...
                    if entry['nested_query']:
                        query = f"""SELECT hxb_inspect.avg_thickness FROM module_assembly  JOIN hxb_inspect  ON REPLACE(module_assembly.hxb_name, '-', '') = REPLACE(hxb_inspect.hxb_name, '-', '')  WHERE REPLACE(module_assembly.module_name, '-', '') = '{module}' AND (module_assembly.xml_upload_success IS NULL OR module_assembly.xml_upload_success = FALSE) ORDER BY hxb_inspect.date_inspect DESC LIMIT 1;"""
                    else:
