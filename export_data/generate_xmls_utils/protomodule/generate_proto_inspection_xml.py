...
async def process_module(conn, yaml_file, xml_file_path, output_dir, date_start, date_end, lxplus_username, partsnamelist=None, skip_uploaded=True):
    ...
    for dbase_table in db_tables:
        if partsnamelist:
            query = f"""SELECT REPLACE(proto_name,'-','') AS proto_name FROM {dbase_table} WHERE proto_name = ANY($1) {skip_filter}"""
            ...
            for entry in module_data:
                ...
                if xml_var in ['LOCATION']:
                    ...
                    if entry['nested_query']:
                        query = entry['nested_query'] + f" WHERE REPLACE(proto_assembly.proto_name,'-','') = '{proto_name}' "
