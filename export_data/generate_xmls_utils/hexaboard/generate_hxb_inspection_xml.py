...
async def process_module(conn, yaml_file, xml_file_path, output_dir, date_start, date_end, lxplus_username, partsnamelist = None, skip_uploaded=True):
    ...
    wb_data = yaml_data['hxb_inspection']
    ...
    if partsnamelist:
        query = f"""SELECT REPLACE(hxb_name,'-','') AS hxb_name FROM hxb_inspect WHERE hxb_name = ANY($1) {skip_filter}"""
    ...
    for hxb_name in hxb_list:
        ...
        try:
            ...
            # 'hexaboard' has one row per part; stamp it directly with the real generation time
            await update_timestamp_col(conn,
                                        update_flag=True,
                                        table_list=['hexaboard'],
                                        column_name='xml_gen_datetime',
                                        part='hexaboard',
                                        part_name=hxb_name,
                                        timestamp=now)
            # 'hxb_inspect' can have multiple rows per part; only stamp the latest with the real time
            latest_row = await fetch_from_db(f"""
                SELECT date_inspect, time_inspect FROM hxb_inspect
                WHERE REPLACE(hxb_name,'-','') = '{hxb_name}'
                ORDER BY date_inspect DESC, time_inspect DESC LIMIT 1
                """, conn)
            if latest_row:
                latest_date, latest_time = latest_row['date_inspect'], latest_row['time_inspect']
                # Stamp every entry for this hexaboard with the -100yr sentinel...
                await update_timestamp_col(conn,
                                            update_flag=True,
                                            table_list=['hxb_inspect'],
                                            column_name='xml_gen_datetime',
                                            part='hexaboard',
                                            part_name=hxb_name,
...
async def main(dbpassword, output_dir, date_start, date_end, lxplus_username, encryption_key = None, partsnamelist=None, skip_uploaded=True):
    ...
    xml_file_path = 'export_data/template_examples/hexaboard/inspection_upload.xml'# XML template file path
